...
export class ReporteComponent implements OnInit {
  ...
  constructor(
    ...
    private _unitidadService: UtilidadService
  ) {
  ...
  buscarVentas(){
    ...
    if(_fechaInicio === "Invalid date" || _fechaFin === "Invalid date"){
      this._unitidadService.mostrarAlerta("Debe de ingresar ambas fechas", "Oops!");
      return;
    }
    ...
    this._ventaService.reporte(_fechaInicio, _fechaFin).subscribe({
      next: (data) =>{
        if (data.status)
        {
          ...
          this._unitidadService.mostrarAlerta("No se encontraron datos", "Oops!");
        }
      },
      error(err) {
  ...
  exportarExcel(){
    ...
    XLSX.writeFile(wb, "Repore Ventas.xlsx");
  }
